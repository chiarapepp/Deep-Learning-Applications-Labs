...
def parse_args():
    ...
    parser.add_argument("--eval-interval", type=int, default=50, help="Evaluate the policy every --eval-interval iterations")
    parser.add_argument("--eval-episodes", type=int, default=20, help="Evaluate the policy for --eval-episodes episodes")
    parser.add_argument("--normalize", action="store_true", help="If true, normalize G_t - b_t to zero mean and unit variance")
    parser.add_argument("--clip-grad", action="store_true", help="If true, clip gradients to unit norm for both the policy and the value networks")
    ...
    parser.add_argument("--t-schedule", choices=["lin", "exp"], help="Choose between a linear or exponential temperature scheduler")
